...
class AuthorDAO(BaseDAO):
    ...
    @classmethod
    async def find_all_authors(cls):
        async with async_session_maker() as session:
            async with session.begin():
                query = select(cls.model).options(
                    selectinload(cls.model.books))
                result = await session.execute(query)

                authors = result.scalars().all()  # Получаем всех авторов
                author_data = []
    ...
    async def find_one_or_none_by_id(cls, author_id: int):
        async with async_session_maker() as session:
            query = select(cls.model).filter_by(
                id=author_id).options(selectinload(cls.model.books))
            result = await session.execute(query)
